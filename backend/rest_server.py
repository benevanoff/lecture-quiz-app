...
def register():
    ...
    if not name:
        return "must provide name", 403  # Check if name provided
    elif not username:
        return "must provide username", 403  # Check if username provided
    elif not email:
        return "must provide email", 403  # Check if email provided
    elif not password:
        return "must provide password", 403  # Check if password provided
    elif any(user_dict["username"] == username for user_dict in usernames):
        return "Username already taken", 403  # Check if username exists
    elif not type in USER_TYPES:
        return (
            "Error: Type was not inputted / is not a valid type.",
            403,
        )  # Check if type is valid
    elif not check_email(email): return "Email is invalid", 406  # Check if email is valid
    elif not checkpass[0]:
        return checkpass[1], 406  # Check if password meets complexity requirements

    sql(
        "INSERT INTO users(name, username, email, hash, type) VALUES (%s, %s, %s, %s, %s)",
...
def login():
    ...
    if (
        not query_result
    ):  # if no user with the requested username found - return code 403
...
def reset_password():
    ...
    sql(
        "UPDATE users SET hash = %s WHERE user_id = %s",
...
def create_lecture():
    ...
    if not request.is_json:
        return "Invalid Input", 422  # Invalid input
    elif not title:
        return "Error: Title was not inputted.", 422  # Title not provided
    elif not body:
        return "Error: Body was not inputted.", 422  # Body not provided
    elif not category in CATEGORIES:
        return (
            "Error: Category was not inputted / is not a valid category.",
            422,
        )  # Invalid category
    ...
    with get_sql_db_connection() as sql_client:
        with sql_client.cursor() as cur:
            cur.execute(
                "INSERT INTO lectures (title, body, category) VALUES (%s, %s, %s)",
                (title, body, category),
            )
            lecture_id = cur.lastrowid

    return (
        jsonify({"lecture_id": lecture_id, "completed": True}),
        200,
    )  # Lecture created
...
def edit_lecture(lecture_id):
    ...
    if not category in CATEGORIES:
        return "Error: Category was not inputted.", 422  # Invalid category

    sql(
        "UPDATE lectures SET title=%s, body=%s, category=%s, modified=CURRENT_TIMESTAMP WHERE lecture_id=%s;",
        (title, body, category, lecture_id),
    )

    return jsonify({"lecture_id": lecture_id, "completed": True}), 200  # Lecture edited
...
# -------------------------------- Get Problemsets List -------------------------------- #
@app.route("/l<int:lecture_id>/problemsets", methods=["GET"])
def get_problemsets(lecture_id):
    query_results = sql(
        "SELECT problemset_id FROM problemsets WHERE lecture_id=%s", lecture_id
    )

    if not query_results:
        return [], 200  # No Problemsets found
    return [
        result["problemset_id"] for result in query_results
    ], 200  # Return Problemset IDs
...
def create_problemset(lecture_id):
    ...
    previouslectureid = sql(
        "SELECT lecture_problemsetid FROM problemsets WHERE lecture_id=%s", lecture_id
    )
    ...
    with get_sql_db_connection() as sql_client:
        with sql_client.cursor() as cur:
            cur.execute(
                "INSERT INTO problemsets (lecture_problemsetid, lecture_id, title, body) VALUES (%s, %s, %s, %s)",
                (lecture_problemsetid, lecture_id, title, body),
            )
            problemset_id = cur.lastrowid

    return (
        jsonify(
            {
                "problemset_id": problemset_id,
                "lecture_problemsetid": lecture_problemsetid,
                "completed": True,
            }
        ),
        200,
...
def get_problemset(lecture_id, lecture_problemsetid):
    ...
    query_result = sql(
        "SELECT * FROM problemsets WHERE problemset_id=%s",
        (getproblemset_id(lecture_id, lecture_problemsetid)),
    )

    if not query_result:
        return [], 200  # Problemset not found
    return jsonify(query_result[0]), 200  # Return Problemset details
...
def edit_problemset(lecture_id, lecture_problemsetid):
    ...
    sql(
        "UPDATE problemsets SET title=%s, body=%s, modified=CURRENT_TIMESTAMP WHERE problemset_id=%s;",
        (title, body, problemset_id),
    )
...
def get_problems(lecture_id, lecture_problemsetid):
    query_results = sql(
        "SELECT problem_id FROM problems WHERE problemset_id=%s",
        getproblemset_id(lecture_id, lecture_problemsetid),
    )

    if not query_results:
        return [], 200  # No problems found
    return [result["id"] for result in query_results], 200  # Return problem IDs


# -------------------------------- Create Problem -------------------------------- #
@app.route(
    "/l<int:lecture_id>/ps<int:lecture_problemsetid>/problem/create", methods=["POST"]
)
@role_required("teacher", "admin")
def create_problem(lecture_id, lecture_problemsetid):
    ...
    previous_problemset_problemid = sql(
        "SELECT problemset_problemid FROM problems WHERE problemset_id=%s", lecture_id
    )
    if not previous_problemset_problemid:
        problemset_problemid = 1
    else:
        problemset_problemid = (
            previous_problemset_problemid[-1]["problemset_problemid"] + 1
        )

    with get_sql_db_connection() as sql_client:
        with sql_client.cursor() as cur:
            cur.execute(
                "INSERT INTO problems (problemset_id, problemset_problemid, question, option1, option2, option3, option4, correct) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)",
                (
                    problemset_id,
                    problemset_problemid,
                    question,
        ...
        jsonify(
            {
                ...
                "problemset_problemid": problemset_problemid,
                "completed": True,
            }
        ),
        200,
...
# -------------------------------- Get Problem -------------------------------- #
@app.route(
    "/l<int:lecture_id>/ps<int:lecture_problemsetid>/p<int:problemset_problemid>",
    methods=["GET"],
)
def get_problem(lecture_id, lecture_problemsetid, problemset_problemid):
    query_result = sql(
        "SELECT * FROM problems WHERE problem_id=%s",
        (getproblem_id(lecture_id, lecture_problemsetid, problemset_problemid)),
    )

    if not query_result:
        return [], 200  # Problem not found
    return jsonify(query_result[0]), 200  # Return problem details


# -------------------------------- Edit Problem -------------------------------- #
@app.route(
    "/l<int:lecture_id>/ps<int:lecture_problemsetid>/p<int:problemset_problemid>/edit",
    methods=["POST"],
)
@role_required("teacher", "admin")
def edit_problem(lecture_id, lecture_problemsetid, problemset_problemid):
    ...
    option4 = request.get_json().get("option4").strip()
    correct = request.get_json().get("correct").strip()
    problem_id = getproblem_id(lecture_id, lecture_problemsetid, problemset_problemid)
    ...
    sql(
        "UPDATE problems SET question=%s, option1=%s, option2=%s, option3=%s, option4=%s, correct=%s, modified=CURRENT_TIMESTAMP WHERE problem_id=%s;",
        (question, option1, option2, option3, option4, correct, problem_id),
    )
...
# -------------------------------- Delete Problem -------------------------------- #
@app.route(
    "/l<int:lecture_id>/ps<int:lecture_problemsetid>/p<int:problemset_problemid>/delete",
    methods=["POST"],
)
@role_required("teacher", "admin")
...
# -------------------------------- Check Problem -------------------------------- #
@app.route(
    "/l<int:lecture_id>/ps<int:lecture_problemsetid>/p<int:problemset_problemid>/check",
    methods=["GET"],
)
